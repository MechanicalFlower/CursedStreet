...
const DIALOGUE_PITCHES = {Nathan = 0.8, Coco = 1}
...
func _ready() -> void:
	...
	character_label.bbcode_text = tr(dialogue.character)

	dialogue_label.rect_size.x = (
		margin.rect_size.x
		- margin.get("custom_constants/margin_left")
		- margin.get("custom_constants/margin_right")
	)
	...
	balloon.rect_global_position = Vector2(
		(viewport_size.x - balloon.rect_size.x) * 0.5,
...
func get_responses() -> Array:
	...
	for child in responses_menu.get_children():
		if "disallowed" in child.name.to_lower():
			continue
		items.append(child)

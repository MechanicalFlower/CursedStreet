...
func _ready():
	...
	for locale in loaded_locales:
		...
		if locale == SettingsManager.get_value("gui", "language"):
			selected = index

	popup.set("custom_fonts/font", load("res://resources/fonts/settings_font.tres"))
	popup.set("custom_styles/hover", load("res://resources/styles/settings_language_hover.tres"))
	popup.set(
		"custom_styles/panel", load("res://resources/styles/settings_language_background.tres")
	)
...
func _on_SettingsLanguageButton_item_selected(id):
	...
	SettingsManager.set_value("gui", "language", locale)
